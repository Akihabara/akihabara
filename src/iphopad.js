	...
	_listen: function(e) {
		...
		for (i in this._cross) {
			if (nc[i] != iphopad._cross[i])
				if (nc[i]) AkihabaraInput._keydown({fake: true, keyCode: AkihabaraInput._keymap[i]});
				else AkihabaraInput._keyup({fake: true, keyCode: AkihabaraInput._keymap[i]});
		}
		for (i in this._buttons) {
			if (nb[i] != iphopad._buttons[i])
				if (nb[i]) AkihabaraInput._keydown({fake: true, keyCode: AkihabaraInput._keymap[i]});
				else AkihabaraInput._keyup({fake: true, keyCode: AkihabaraInput._keymap[i]});
		}
